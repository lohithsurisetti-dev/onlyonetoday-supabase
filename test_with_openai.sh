#!/bin/bash
# Complete Backend Test with OpenAI Embeddings
...
echo "🧪 Complete Backend Testing with OpenAI Embeddings"
echo "===================================================="
echo ""
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo "Usage: ./test_with_openai.sh YOUR_OPENAI_API_KEY"
    echo ""
...
echo -e "${BLUE}Step 1: Testing OpenAI API Connection${NC}"
echo "========================================"
echo ""

# Test OpenAI API key
TEST_RESPONSE=$(curl -s -X POST "https://api.openai.com/v1/embeddings" \
...
if echo "$TEST_RESPONSE" | grep -q "embedding"; then
    ...
    TOKENS=$(echo "$TEST_RESPONSE" | python3 -c "import sys, json; print(json.load(sys.stdin)['usage']['total_tokens'])" 2>/dev/null || echo "1")
    echo "   Tokens used: $TOKENS"
    echo "   Cost: ~$0.00000002"
else
...
export SUPABASE_OPENAI_API_KEY=$OPENAI_API_KEY
...
if echo "$RESPONSE1" | grep -q '"success":true'; then
    ...
    echo -e "${GREEN}✅ SUCCESS: Post created with OpenAI embeddings${NC}"
...
echo "===================================================="
echo ""
echo "✅ OpenAI API integration working"
